...
# Handle column transformations completely based on scaler constraints
if expected_features == 10:
    ...
    # Handles 8 features seamlessly if the target column was scaled with numericals
    input_vector_8 = np.array([[temperature, humidity, wind_speed, air_quality, rainfall, uv_index, visibility, 0.0]])
    scaled_8 = data_scaler.transform(input_vector_8)
    scaled_numerical = scaled_8[0][:7]
    scaled_input = np.hstack([scaled_numerical, [weather_encoded, comfort_encoded, season_encoded]]).reshape(1, -1)
    predicted_score = model_to_use.predict(scaled_input)[0]
else:
    st.error(f"⚠️ Unexpected Scaler configuration: expected {expected_features} features.")
    st.stop()

# --- THE MASTER FIX FOR TARGET SCALING ---
target_mean = 7.338278
target_std = 1.446426

if -5.0 <= predicted_score <= 5.0:
    unscaled_score = target_mean + (predicted_score * target_std)
else:
    unscaled_score = predicted_score

# Post-processing prediction constraints
final_score = max(0.0, min(10.0, unscaled_score))
